"""builder for pems-bay traffic forecasting dataset.

source: dcrnn data release
files:
  pems-bay.h5     325 sensors x 52116 timestamps (5min) speed matrix
  adj_mx_bay.pkl  [sensor_ids_list, id_to_idx_dict, adj_mx (325,325)]
...
def _load_raw(raw_dir: str):
    """read pems-bay.h5 (speeds) and adj_mx_bay.pkl (adjacency)."""
    import h5py

    raw_path = Path(raw_dir)
    h5_path = raw_path / "pems-bay.h5"
    adj_path = raw_path / "adj_mx_bay.pkl"
    ...
    with h5py.File(str(h5_path), "r") as f:
        speeds = f["speed/block0_values"][:]  # [T, N]
        # axis0 may be int64 (pems-bay) or bytes (metr-la); coerce to str for matching
        sensor_ids_h5 = [s.decode() if isinstance(s, bytes) else str(s)
                         for s in f["speed/axis0"][:]]

    with open(adj_path, "rb") as f:
        sensor_ids_adj, _id_to_idx, adj = pickle.load(f, encoding="latin1")

    sensor_ids_adj = [str(s) for s in sensor_ids_adj]
...
def build_pemsbay_graphs(raw_dir: str, hour_window: int = 12, hour_stride: int = 12,
                         adj_threshold: float = 0.0):
    """build pems-bay temporal graph snapshots.

    args:
        raw_dir: directory containing pems-bay.h5 and adj_mx_bay.pkl
