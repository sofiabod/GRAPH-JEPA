...
from src.data.pemsbay_builder import build_pemsbay_graphs


def _write_fake_raw(raw_dir, n_nodes=12, n_timestamps=24 * 12 * 30, seed=0):
    """write fake pems-bay.h5 and adj_mx_bay.pkl into raw_dir.

    h5 stores a dataframe-style speed matrix of shape [n_timestamps, n_nodes]
    under the 'speed' group (matching the dcrnn pems-bay layout).
    ...
    h5_path = raw_dir / "pems-bay.h5"
    with h5py.File(str(h5_path), "w") as f:
        grp = f.create_group("speed")
        # mimic the dcrnn pems-bay h5 layout: axis0=columns, block0_values=data
    ...
    adj_path = raw_dir / "adj_mx_bay.pkl"
...
def test_output_is_list_of_data(tmp_path):
    ...
    graphs, meta = build_pemsbay_graphs(str(tmp_path))
...
def test_missing_raw_files_raises(tmp_path):
    with pytest.raises(FileNotFoundError):
        build_pemsbay_graphs(str(tmp_path))


def test_n_nodes_matches_sensor_count(tmp_path):
    # builder must report the exact number of sensors present in adj_mx_bay
    _write_fake_raw(tmp_path, n_nodes=325, n_timestamps=24 * 12 * 8)
    graphs, meta = build_pemsbay_graphs(str(tmp_path))
    assert meta["n_nodes"] == 325
    for g in graphs:
        assert g.x.shape[0] == 325
